...
The site currently uses `site/assets/img/about/logo.png` as a universal fallback for any missing logo/icon.

| Asset type | Where to store | File name | Used by | Notes |
|---|---|---|---|---|
| Site logo (square) | `site/assets/img/about/` | `logo.png` | Header brand + fallbacks | Already present. Prefer ~512×512 PNG with transparent background. |
...
| App icon (per project) | `site/assets/img/about/` (or new folder `site/assets/apps/<slug>/`) | e.g. `chronomo.png` | Projects list + Project detail hero | Currently using `logo.png` everywhere; we can switch to per-project icons when provided. |
...
| Social preview image (optional) | `site/assets/img/about/` | `og-image.png` | `<meta property="og:image">` (future) | Not wired yet; recommended for sharing. |
